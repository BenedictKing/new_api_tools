...
func TestRefundTopUpUpdatesQuotaAndStatus(t *testing.T) {
	db := installSQLiteForTests(t)
	for _, stmt := range []string{
		`CREATE TABLE users (id INTEGER PRIMARY KEY, quota INTEGER NOT NULL)`,
		`CREATE TABLE top_ups (id INTEGER PRIMARY KEY, user_id INTEGER NOT NULL, amount INTEGER NOT NULL, status TEXT)`,
		`INSERT INTO users (id, quota) VALUES (10, 1000)`,
		`INSERT INTO top_ups (id, user_id, amount, status) VALUES (20, 10, 300, 'success')`,
	} {
	...
	if err := RefundTopUp(20); err != nil {
		t.Fatalf("refund failed: %v", err)
	}

	var quota int64
	if err := db.Get(&quota, `SELECT quota FROM users WHERE id = 10`); err != nil {
		t.Fatalf("read quota: %v", err)
	}
	if quota != 700 {
		t.Fatalf("quota should be reduced to 700, got %d", quota)
	}

	var status string
	if err := db.Get(&status, `SELECT status FROM top_ups WHERE id = 20`); err != nil {
		t.Fatalf("read status: %v", err)
	}
	if status != "refunded" {
		t.Fatalf("status should be refunded, got %q", status)
	}

	if err := RefundTopUp(20); err != nil {
		t.Fatalf("second refund should be idempotent: %v", err)
	}
	if err := db.Get(&quota, `SELECT quota FROM users WHERE id = 10`); err != nil {
		t.Fatalf("read quota after second refund: %v", err)
	}
	if quota != 700 {
		t.Fatalf("second refund should not reduce quota again, got %d", quota)
	}
}
