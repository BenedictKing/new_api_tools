...
func RegisterIPMonitoringRoutes(r *gin.RouterGroup) {
	...
	compat := r.Group("/ip-monitoring")
	{
		compat.GET("/stats", GetIPStats)
		compat.GET("/shared", GetSharedIPs)
		compat.GET("/shared-ips", GetSharedIPs)
		compat.GET("/multi-ip-tokens", GetMultiIPTokens)
		compat.GET("/multi-ip-users", GetMultiIPUsers)
		compat.POST("/enable-all-recording", EnableAllIPRecording)
		compat.POST("/enable-all", EnableAllIPRecording)
		compat.GET("/lookup/:ip", LookupIPUsers)
		compat.GET("/users/:user_id/ips", GetUserIPs)
		compat.GET("/indexes", GetIPIndexStatus)
		compat.GET("/index-status", GetIPIndexStatus)
		compat.POST("/indexes/ensure", EnsureIPIndexes)
		compat.POST("/ensure-indexes", EnsureIPIndexes)
		compat.GET("/geo/:ip", GetIPGeo)
		compat.POST("/geo/batch", GetIPGeoBatch)
	}
}
